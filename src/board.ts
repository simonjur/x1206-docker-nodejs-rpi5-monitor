...
export class BoardReader {
    bus: i2c.PromisifiedBus | null = null;
    chip: gpiod.Chip | null = null;
    pldLine: gpiod.Line | null = null;

    public constructor() {
        const rawBus = i2c.openSync(I2C_BUS_NUMBER);
        this.bus = rawBus.promisifiedBus();
    ...
    public async getInfo() {
        ...
        voltage = await readVoltage(this.bus);
        capacity = await readCapacity(this.bus);
        const batteryStatus = getBatteryStatus(voltage);
